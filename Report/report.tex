...
\begin{document}
	...
	p(\boldsymbol{x}) = \sum_{k=1}^{K} \pi_{k} \mathcal{N} \left( \boldsymbol{x}  | \boldsymbol{\mu}_{k}, \Sigma_{k} \right)
 	...
 	To present a world map clustered according to the Gaussian mixture model, we decided to rely on the \textit{Bayesian Information Criterion}, or BIC, to select the best $n$. This was done because BIC is widely employed when dealing with this model and to exploit its probabilistic meaning. 
